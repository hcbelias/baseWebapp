...
//node npm package audition - node security
//gulp flow will not stop if some vulnerabilities have been found
...
gulp.task('copy:resumeFiles', () => {
    return gulp.src([
        `${serverPath}/views/resume/resume.jade`,
        `${serverPath}/components/template.docx`
    ], {cwdbase: true})
        .pipe(gulp.dest(`${paths.dist}`));
});
